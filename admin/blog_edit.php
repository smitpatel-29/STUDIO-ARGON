...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Ensure slug doesn't conflict with other posts
    ...
    if (!empty($title) && !empty($category) && !empty($image)) {
        // We keep the existing author/author_img from the database or update them to current if preferred. 
        // For now, we just don't touch them in the update statement or we can update them to session admin.
        $stmt = $pdo->prepare("UPDATE blog_posts SET title = ?, slug = ?, excerpt = ?, image = ?, category = ?, tag = ?, content = ? WHERE id = ?");
        if ($stmt->execute([$title, $slug, $excerpt, $image, $category, $tag, $content, $id])) {
...
<div class="card" style="max-width: 1000px; margin: 0 auto;">
    <div class="card-header">
        <h2 class="card-title">Edit Post: <?php echo htmlspecialchars($data['title'] ?? 'N/A'); ?></h2>
    ...
    <form method="POST" id="blogForm" style="display: grid; gap: 2rem;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 1.5rem;">
            <div class="form-group">
                ...
                <select name="category" class="form-control" required>
                    ...
                    foreach ($cats as $c) {
                        $selected = ($c['slug'] == $data['category']) ? 'selected' : '';
            ...
            <div class="form-group">
                ...
                <input type="text" name="tag" value="<?php echo htmlspecialchars($data['tag'] ?? ''); ?>" class="form-control" required>
            </div>
            <div class="form-group">
                <label style="display: block; margin-bottom: 0.6rem; font-size: 0.8rem; text-transform: uppercase;">Featured Image*</label>
                <div style="display: flex; gap: 0.5rem;">
                    <input type="text" name="image" id="thumbnail-path" value="<?php echo htmlspecialchars($data['image'] ?? ''); ?>" class="form-control" required>
        ...
        <div class="form-group">
            <label style="display: block; margin-bottom: 0.6rem; font-size: 0.8rem; text-transform: uppercase;">Excerpt</label>
            <textarea name="excerpt" class="form-control" rows="2"><?php echo htmlspecialchars($data['excerpt'] ?? ''); ?></textarea>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="wysiwyg-container" style="border: 1px solid var(--border); border-radius: 12px; overflow: hidden; background: #fff;">
                ...
                <div id="editor" contenteditable="true" style="min-height: 400px; padding: 25px; outline: none; font-size: 1.1rem; line-height: 1.6;"><?php echo $data['content']; ?></div>
